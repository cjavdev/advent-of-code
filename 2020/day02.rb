...
p input
  .map {|line| line.chomp.split(%r{[- :]+})}
  .map {|min, max, char, password| [min.to_i, max.to_i, char, password]}
  .count {|min, max, char, password| password[min - 1].include?(char) ^ password[max-1].include?(char)}
  # .count {|min, max, char, password| (min..max).include?(password.count(char))}
  # .count {|min, max, char, password| password[min - 1..max-1].include?(char)}
